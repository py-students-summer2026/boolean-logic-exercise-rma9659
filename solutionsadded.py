...
def is_sweltering():
    temp = int(input("Enter the current temperature in Farenheit: "))
    return temp > 90
    """
...
def is_humid():
    answer = input("Is it humid today? (yes or no): ")
...
def is_inclement():
    forecast = input("What is the weather forecast for today? ").lower().strip()
    return forecast in ["raining", "snowing", "sleeting"]
